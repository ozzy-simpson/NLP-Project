...
public class Sentence{
    ...
    public ArrayList<String> splitSentence(String sentence) {
        ...
        // Removes instances of stopwords from words
        for (int i = 0; i < words.size(); i++) {
            for (int j = 0; j < stopwords.length; j++) {
                if (words.get(i).toLowerCase() == Driver.removePunct(stopwords[j])) { // Make sure they are equal when punctuation removed and case is the same
                    words.remove(i);
                }
            }
        }
